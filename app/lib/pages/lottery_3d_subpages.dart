...
class _Lottery3DAccountPageState extends State<Lottery3DAccountPage> {
  ...
  Future<void> _load() async {
    final u = await AuthService.instance.getStoredUser() ?? <String, dynamic>{};
  ...
  @override
  Widget build(BuildContext context) {
    final name = _pick(_user, ['userName', 'username', 'name', 'fullName', 'phone'], fallback: '-');
    final userId = _pick(_user, ['id', '_id', 'userId'], fallback: '-');
...
class _Lottery3DQuizPageState extends State<Lottery3DQuizPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final drawNext = '${_slotData?['drawLabelNext'] ?? _slotLabel(_slotData?['nextSlotStartIso'])}';
          ...
          children: [
            Row(
              children: [
                ...
                SizedBox(
                  width: 90,
                  child: _drawSlotChip(drawPrev, false),
                ),
                const SizedBox(width: 4),
                SizedBox(
                  width: 90,
                  child: _drawSlotChip(draw, true),
                ),
                const SizedBox(width: 4),
                SizedBox(
                  width: 90,
                  child: _drawSlotChip(drawNext, false),
                ),
                const SizedBox(width: 8),
                SizedBox(
                  height: 28,
...
class _QuizQuestionList3D extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      itemCount: questions.length > 25 ? 25 : questions.length,
      separatorBuilder: (_, __) => const SizedBox(height: 6),
      itemBuilder: (context, i) {
        final q = questions[i];
        final options = q['options'] is Map ? Map<String, dynamic>.from(q['options'] as Map) : <String, dynamic>{};
        return Container(
          padding: const EdgeInsets.all(8),
          decoration: BoxDecoration(
            color: Colors.white,
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 4,
                children: [
                  Text('A: ${options['A'] ?? ''}', style: const TextStyle(fontSize: 11, color: Colors.black)),
                  Text('B: ${options['B'] ?? ''}', style: const TextStyle(fontSize: 11, color: Colors.black)),
                  Text('C: ${options['C'] ?? ''}', style: const TextStyle(fontSize: 11, color: Colors.black)),
                  Text('D: ${options['D'] ?? ''}', style: const TextStyle(fontSize: 11, color: Colors.black)),
                ],
              ),
            ],
          ),
                        ...
                        return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Text('Slot: ${row['slotLabel']}', style: const TextStyle(fontWeight: FontWeight.w800)),
                                  const SizedBox(width: 10),
                                  Text('Points: ${row['totalPoints']}', style: const TextStyle(fontWeight: FontWeight.w700)),
                                  const SizedBox(width: 10),
                                  Text('Pending: ${row['pendingCount']}', style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFFD97706))),
                                ],
                              ),
                              const SizedBox(height: 6),
                              ...bets.take(8).map(
...
class _Lottery3DHistoryViewState extends State<_Lottery3DHistoryView> {
  ...
  Future<void> _load() async {
    ...
    try {
      final user = await AuthService.instance.getStoredUser();
      final userName = (user?['userName'] ?? user?['name'] ?? user?['fullName'] ?? 'user')
          .toString()
          .trim();
      final headers = await _authHeaders();
      if (headers.isEmpty) {
      ...
      final uri = Uri.parse('$kApiBaseUrl/quiz/my-quiz-bets?limit=$_limit&mode=3d');
      ...
      setState(() {
        ...
        _rows = mapped;
        _lastApiInfo = 'success=true, items=${list.length}, grouped=${mapped.length}';
        _error = mapped.isEmpty
                ...
                Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<int>(
                      ...
                      items: const [50, 100, 200, 500, 1000, 5000]
